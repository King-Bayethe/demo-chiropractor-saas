...
SELECT setval(pg_get_serial_sequence('public.form_submissions', 'id'), 1, false) WHERE pg_get_serial_sequence('public.form_submissions', 'id') IS NOT NULL;

-- Add a comment to track when cleanup was performed
COMMENT ON SCHEMA public IS 'Database cleaned on ' || now()::text;