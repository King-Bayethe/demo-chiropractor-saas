-- Fix date handling in create_patient_from_form_submission function
  ...
  IF patient_id_var IS NOT NULL THEN
    ...
    SET
      ...
      date_of_birth = COALESCE(
        CASE 
          WHEN form_data_parsed->>'dateOfBirth' IS NOT NULL AND form_data_parsed->>'dateOfBirth' != ''
          THEN (form_data_parsed->>'dateOfBirth')::date
          WHEN form_data_parsed->>'dob' IS NOT NULL AND form_data_parsed->>'dob' != ''
          THEN (form_data_parsed->>'dob')::date
          ELSE NULL
        END, 
        date_of_birth
      ),
      gender = COALESCE(form_data_parsed->>'gender', form_data_parsed->>'sex', gender),
      ...
      -- Accident details for PIP cases - Fix date handling
      accident_date = COALESCE(
        CASE 
          WHEN form_data_parsed->>'accidentDate' IS NOT NULL AND form_data_parsed->>'accidentDate' != ''
  ...
  ELSE
    ...
    INSERT INTO public.patients (
      ...
      CASE 
        ...
        ELSE NULL
      END,
      COALESCE(form_data_parsed->>'gender', form_data_parsed->>'sex'),
